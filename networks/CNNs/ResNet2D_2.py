...
class ResNet2D_2(nw.NetworkBase):
    def __init__(self, model_depth: int, input_dim: int,
    ...
    def __make_layers(self, block_ctor, plane_num: int, layer_num: int, shortcut_type: str, stride: int = 1):
        ...
        if stride != 1 or self.in_plane_num != expanding_plane_num:
            if shortcut_type == 'A':
                down_sample_func = ft.partial(ResNet2D_2.__down_sample_basic_block, plane_num=expanding_plane_num, stride=stride)
            else:
