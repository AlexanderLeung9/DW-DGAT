...
class ChAdaViT(nw.NetworkBase):
    ...
    def __init__(self, params: nw.NetworkParams):
        ...
        assert isinstance(params.args, ag.ChAdaViTArgs)
        args = params.args
        self.args = args

        # Embeddings dimension
        self.num_features = self.embed_dim = args.embed_dim

        # Num of maximum channels in the batch
        self.max_channels = args.max_number_channels

        # Tokenization module
        self.token_learner = TokenLearner(img_size=ag.BDArguments.ROI_NUM, patch_size=args.patch_size, in_chans=args.feature_num, embed_dim=self.embed_dim)
        ...
        self.blocks = nn.ModuleList([
            TransformerEncoderLayer(d_model=self.embed_dim, nhead=args.num_heads, dim_feedforward=2048, dropout=dpr[i], batch_first=True)
            for i in range(args.depth)
        ])
        self.norm = nn.LayerNorm(self.embed_dim)

        # Classifier head
        self.head = nn.Linear(self.embed_dim, args.class_num) if args.class_num > 0 else nn.Identity()

        # Return only the [CLS] token or all tokens
        self.return_all_tokens = args.return_all_tokens
    ...
    def channel_aware_tokenization(self, x, max_channels=10):
        B, nc, w, h = x.shape    # (B*num_channels, 1, w, h)

        # Tokenize through linear embedding
        tokens_per_channel = self.token_learner(x)

        list_num_channels = [1] * B
        # Concatenate tokens per channel in each image
        chunks = torch.split(tokens_per_channel, list_num_channels, dim=0) # List of (img_channels, embed_dim)
    ...
    def forward(self, x):
        ...
        x, channel_mask = self.channel_aware_tokenization(x) # (B*num_channels, embed_dim)
        ...
        if self.return_all_tokens:
            ...
            tokens = x[:, 0]  # return only the [CLS] token
            scores = self.classifier.forward(tokens)
            return scores
