...
from .graphsage_layer import GraphSageLayer, DenseGraphSage
...
def masked_softmax(matrix, mask, dim=-1, memory_efficient=True,
    ...
    if mask is None:
        ...
        if not memory_efficient:
            result = F.softmax(matrix * mask, dim=dim)
            result = result * mask
            result = result / (result.sum(dim=dim, keepdim=True) + 1e-13)
        else:
            masked_matrix = matrix.masked_fill((1 - mask).byte(), mask_fill_value)
            result = F.softmax(masked_matrix, dim=dim)
    return result
...
class ContrastPoolLayer(nn.Module):
    ...
    def __init__(self, input_dim, assign_dim, output_feat_dim,
                 activation, dropout, aggregator_type, link_pred, batch_norm: bool, pool_assign='GraphSage', max_node_num=0):
        super().__init__()
