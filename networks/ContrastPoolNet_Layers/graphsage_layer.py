...
class GraphSageLayer(nn.Module):
    def __init__(self, in_feats, out_feats, activation, dropout,
                 aggregator_type: str, batch_norm: bool, residual: bool = False,
    ...
    @staticmethod
    def copy_src(node_feat: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """
        模拟 dgl.function.copy_src('h', 'm') 的行为：
        给定节点特征和边索引，从源节点复制特征作为边的消息。

        参数：
            node_feat: Tensor of shape [num_nodes, feature_dim]
            edge_index: LongTensor of shape [2, num_edges], where
                        edge_index[0] 是源节点索引，
                        edge_index[1] 是目标节点索引。

        返回：
            msg: Tensor of shape [num_edges, feature_dim]
        """
        src = edge_index[0]  # 源节点索引
        msg = node_feat[src]  # 从源节点复制特征
        return msg

    @staticmethod
    def u_mul_e(node_feat: torch.Tensor, edge_feat: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        """
        模拟 dgl.function.u_mul_e('h', 'w', 'm') 的行为。

        参数：
            node_feat: Tensor [num_nodes, feat_dim]，节点特征 h
            edge_feat: Tensor [num_edges, feat_dim]，边特征 w
            edge_index: Tensor [2, num_edges]，边的起点和终点索引

        返回：
            msg: Tensor [num_edges, feat_dim]，每条边的消息 m = h_src * w
        """
        src_nodes = edge_index[0]  # 每条边的源节点索引
        src_feat = node_feat[src_nodes]  # 每条边源节点的特征 h_i
        msg = src_feat * edge_feat  # 逐元素乘法
        return msg
